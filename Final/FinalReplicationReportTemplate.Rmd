...
B <- cbind(FinalData$Meditator, FinalData$Age, FinalData$Gender, FinalData$Ethnicity)
B <- as.data.frame(B)
...
* Attempt to replicate the other plots in Koopmann-Holm et al.'s Figure 1.
* Investigate Ideal LAP differences by US region (primarily West Coast versus East Coast). 
```{r echo=F}
# ANCOVA for group_by Region. This should still control for Age and relevent other affect. 
```
* Look at ideal affects by ethnic identification based on previous literature.
```{r echo=F}
# ANCOVA for group_by Ethnicity, in both these cases controlling for Age and Actual affect.
```
...
* Compare the average number of years that Koopmann-Holm et al.'s and our Meditators practiced.
```{r}

```

* Also compare the average duration of Meditators' practice sessions and compare with Koopamnn-Holm's. This could be a partial cause of the failure to replicate.
...
```
* Double check that my n() is not messing anything up. I'm not confident about it.
